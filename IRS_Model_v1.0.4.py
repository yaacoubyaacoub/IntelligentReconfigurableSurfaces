...
def show_phase_shift_plots(phase_shifts, title, save_plot=False, results_directory_path=None, subplot_position=None):
    """
...
def model(transmitter, receiver, room_sizes):
    ...
    output = '\n'.join([
        ...
        f"Average NLOS distance between emitter and receiver through surface: {average_total_distance} m",
        f"Incident angle (in degrees): {np.round(np.degrees(theta_i[(math.floor(theta_i.shape[0] / 2)), (math.floor(theta_i.shape[1] / 2))]), 2)}°",
        f"Theoretical reflected angle (in degrees): {np.round(np.degrees(theta_r[(math.floor(theta_r.shape[0] / 2)), (math.floor(theta_r.shape[1] / 2))]), 2)}°",
        f"Real reflected angle (in degrees): {np.round(np.degrees(real_theta_r[(math.floor(real_theta_r.shape[0] / 2)), (math.floor(real_theta_r.shape[1] / 2))]), 2)}°",
        f"Theoretical Phi angle (in degrees): {np.round(np.degrees(phi_r[(math.floor(phi_r.shape[0] / 2)), (math.floor(phi_r.shape[1] / 2))]), 2)}°",
        f"Real Phi angle (in degrees): {np.round(np.degrees(real_phi_r[(math.floor(real_phi_r.shape[0] / 2)), (math.floor(real_phi_r.shape[1] / 2))]), 2)}°",
        ...
        f"Noise Power (in dBm): {round(10 * math.log10(noise_power / 1e-3), 2)} dBm",
        f"Signal to Noise ratio (SNR) (in dB): {round(10 * math.log10(received_power / noise_power), 2)} dB",
